...
def trainer_setup(
    ...
    if use_wandb:
        assert (
            proj_name is not None
        ), "proj_name should not be None when use_wandb is True"
        assert (
            data_name is not None
        ), "data_name should not be None when use_wandb is True"
    ...
    if accelerator == "gpu" and len(devices) > 1:
        # initilize trainer.
        trainer = pl.Trainer(
            logger=False,  # HACK: tensorboard logger throws errors
...
def test(
    ...
    test_loss = trainer.test(model, dataloaders=test_dataloader, ckpt_path=ckpt_path)
