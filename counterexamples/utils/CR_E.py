...
def color_reassignment(
    color_text_l: dict, color_text_r: dict, Gl: nx.Graph, Gr: nx.Graph
):
...
def CR_E(
    Gl: nx.Graph, Gr: nx.Graph, verbose: bool = False
) -> Tuple[bool, Optional[dict], Optional[dict], int]:
    if verbose:
        print("-----------Beginning CR_E-----------")
    iteration = 0

    label_list_l, label_list_r = (
        nx.get_node_attributes(Gl, "label"),
        nx.get_node_attributes(Gr, "label"),
    )
    if verbose:
    ...
    while True:
        ...
        new_label_list_l, new_label_list_r = color_reassignment(
            color_text_l, color_text_r, Gl, Gr
        )
        iteration += 1
...
def CR_E_once(Gl: nx.Graph, Gr: nx.Graph, verbose: bool = False) -> Tuple[bool, bool]:
    ...
    color_kind_num_l_before, color_kind_num_r_before = (
        len(set(label_list_l.values())),
        len(set(label_list_r.values())),
    )
    if verbose:
        print("-----------Labels Before-----------")
        print("Graph L:", label_list_l)
    ...
    color_kind_num_r_after, color_kind_num_l_after = (
        len(set(new_label_list_l.values())),
        len(set(new_label_list_r.values())),
    )
    ...
    both_stable = (
        color_kind_num_l_before == color_kind_num_l_after
        and color_kind_num_r_before == color_kind_num_r_after
    )
