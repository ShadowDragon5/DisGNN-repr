...
def get_complementary_policy(
    sample_policy: Tuple[List[int], List[int]],
...
def PointCloud2nxGraph(PC: np.array, initial_labels: Optional[dict] = None) -> nx.Graph:
    ...
    """
    DG = np.sqrt(np.sum((PC[:, np.newaxis, :] - PC[np.newaxis, :, :]) ** 2, axis=2))

    G = nx.from_numpy_array(DG, create_using=nx.Graph)
...
def visualize(
    labels_l_dict: Dict[int, int],
    labels_r_dict: Dict[int, int],
    PCl: np.array,
    PCr: np.array,
    com_PCl: np.array,
    com_PCr: np.array,
    ...
    labels_l_list, labels_r_list = (
        list(labels_l_dict.values()),
        list(labels_r_dict.values()),
    )
    color_list_l, color_list_r = (
        [rainbow_colors[i] for i in labels_l_list],
        [rainbow_colors[i] for i in labels_r_list],
    )
