...
from layers.basic_layers import Dense, Residual
...
class TwoFDisInit(nn.Module):
    def __init__(
        ...
        **_,
        ...
        self.ef_lin = Dense(
            in_features=ef_dim, out_features=k_tuple_dim, bias=False, activation_fn=None
        )

        self.pattern_embedding = nn.Embedding(
            num_embeddings=3, embedding_dim=k_tuple_dim, padding_idx=0
        )

        self.mix_lin = Residual(
            hidden_dim=k_tuple_dim, activation_fn=activation_fn, mlp_num=2
        )
    ...
    def forward(
        ...
        idx = torch.arange(N)
        tuple_pattern = torch.ones(
            size=(B, N, N), dtype=torch.int, device=z_mixed.device
        )
