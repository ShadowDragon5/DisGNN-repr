...
class SincRadialBasis(nn.Module):
    # copied from Painn
    def __init__(self, num_rbf, rbound_upper, rbf_trainable=False, **_):
        super().__init__()
        if rbf_trainable:
    ...
    def forward(self, r):
        n = self.n
        output = (math.pi) * n * torch.sinc(n * r)  # type: ignore
        return output
...
class BesselBasisLayer(torch.nn.Module):
    ...
    def forward(self, dist):
        ...
        dist (B, 1)
        """
        return ((self.freq * dist).sin() / (dist + 1e-7)) * (
            (2 / self.rbound_upper) ** 0.5
        )


class GaussianSmearing(nn.Module):
    def __init__(
        self, num_rbf, rbound_upper, rbound_lower=0.0, rbf_trainable=False, **_
    ):
    ...
    def reset_parameters(self):
        offset, coeff = self._initial_params()
        self.offset.data.copy_(offset)  # type: ignore
        self.coeff.data.copy_(coeff)  # type: ignore
...
class NewExpNormalSmearing(nn.Module):
    ...
    def _initial_params(self):
        ...
        means = torch.linspace(start_value, end_value, self.num_rbf)  # type: ignore
        betas = torch.tensor(
    ...
    def forward(self, dist):
        return torch.exp(-self.betas * torch.square((torch.exp(-dist) - self.means)))  # type: ignore
...
rbf_class_mapping = {
    ...
    "nexpnorm": NewExpNormalSmearing,  # RBF
